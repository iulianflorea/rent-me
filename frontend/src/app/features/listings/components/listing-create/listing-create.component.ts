...
export class ListingCreateComponent implements OnInit, OnDestroy {
  ...
  submit(): void {
    ...
    if (this.selectedFiles.length === 0) { this.notifications.error('listing.create.noImages'); return; }
    this.loading = true;
    ...
    this.listingsApi.create(payload).subscribe({
      next: (listing) => {
        this.listingsApi.uploadImages(listing.id, this.selectedFiles).subscribe({
          next: () => {
            this.listingsApi.publish(listing.id).subscribe({
              next: () => {
                this.notifications.success('listing.create.success');
